 ...
 * Description: Sets ALT from attachment Title/filename on upload, fills empty Media Library alts, guarantees alt at render, and provides a one-click content migration to write missing alts into posts/pages.
 * Version: 1.1.0
 * Author: 7on
 */
...
// If user edits attachment Title later, and alt is still empty — fill it
...
/** =============================================================
 * 2) Render-time guarantees (front-end)
 *    - Gutenberg core/image
 *    - Classic content <img class="wp-image-123">
 *    (does NOT modify DB — just ensures alt appears in HTML output)
 * ============================================================= */
add_filter('render_block', function($content, $block){
    ...
    // Non-empty alt already in HTML? keep
    if ( preg_match('~\balt\s*=\s*("([^"]*)"|\'([^\']*)\'|([^\s>]+))~i', $content, $m) ) {
        $val = $m[2] !== '' ? $m[2] : ($m[3] !== '' ? $m[3] : $m[4]);
        if ( trim($val) !== '' ) return $content;
        // alt exists but empty → replace once
        return preg_replace('~\balt\s*=\s*(["\'])\s*\1~', ' alt="'.esc_attr($alt).'"', $content, 1);
    }
    // No alt at all → insert into first <img>
...
add_filter('the_content', function($html){
    $new = preg_replace_callback('~<img\b[^>]*>~i', function($m){
        $tag = $m[0];
        // Non-empty alt present? leave it
        ...
        if ( preg_match('~wp-image-(\d+)~', $tag, $idm) ) {
            ...
            if ( $alt !== '' ) {
                // alt="" → alt="…"
                if ( preg_match('~\balt\s*=\s*(["\'])\s*\1~', $tag) ) {
                    return preg_replace('~\balt\s*=\s*(["\'])\s*\1~', ' alt="'.esc_attr($alt).'"', $tag, 1);
                }
                // no alt attribute → inject before >
...
/** =============================================================
 * 3) Tools page: bulk Media alts + content migration
 * ============================================================= */
...
function tfc_fill_alts_page(){
    ...
    // 3a) Bulk fill alts in Media Library
    ...
    // 3b) Content migration: write missing alts into post/page HTML
    ...
    <div class="wrap">
        ...
        <p style="opacity:.7">Безопасно: существующие непустые ALT нигде не перезаписываются.</p>
    </div>
    <?php
}

/** =============================================================
 * 4) Content fixer: utility used by the Tools page
 * ============================================================= */
function tfc_fix_alts_in_post_content( $content ){
    ...
    $new = preg_replace_callback('~<img\b[^>]*>~i', function($m) use (&$imgs, &$changed){
        $tag = $m[0];
        // If non-empty alt exists, keep tag
        if ( preg_match('~\balt\s*=\s*("([^"]*)"|\'([^\']*)\'|([^\s>]+))~i', $tag, $mm) ) {
            $val = $mm[2] !== '' ? $mm[2] : ($mm[3] !== '' ? $mm[3] : $mm[4]);
            if ( trim(html_entity_decode($val)) !== '' ) return $tag;
        }
        // Resolve attachment id from class wp-image-123
        if ( preg_match('~wp-image-(\d+)~', $tag, $idm) ) {
            $alt = tfc_get_alt_by_attachment( (int)$idm[1] );
            if ( $alt !== '' ) {
                if ( preg_match('~\balt\s*=\s*(["\'])\s*\1~', $tag) ) {
                    $tag = preg_replace('~\balt\s*=\s*(["\'])\s*\1~', ' alt="'.esc_attr($alt).'"', $tag, 1);
